...
public class BusiConfigAddWizard extends Wizard {
	...
	public BusiConfigAddWizard(BSOperationNode operationNode, Table table) {
		this.operationNode = operationNode;
		this.svcInfo = operationNode.getBusiSvcInfo();
		this.table = table;
	...
	public void addPages() {
		this.addPage(new BusiConfigAddWizardPage(this.svcInfo));
	}
	...
	public boolean performFinish() {
		...
		if (null != pages && pages.length > 0) {
			try {
				BusiConfigAddWizardPage page = (BusiConfigAddWizardPage) pages[0];
				String configName = page.getConfigName();
				String fileFolderStr = new StringBuilder("/").append(operationNode.getProjectName())
						.append("/dev/services/busi/").append(operationNode.getBusiSvcInfo().getId()).append("/")
						.toString();
