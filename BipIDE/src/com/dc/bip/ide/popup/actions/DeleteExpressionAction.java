...
public class DeleteExpressionAction implements IObjectActionDelegate {
	...
	public void run(IAction arg0) {
		...
		try {
			File protocolInFile = new File(node.getExpressionService().getMenuFilePath());
		...
		IEditorPart editor;
		if(node.getExpressionService().getLocalFilePath() != null && !"".equals(node.getExpressionService().getLocalFilePath())){
			IPath sdaPath = new Path(node.getExpressionService().getLocalFilePath());
