...
public class ProtocolOutEditor extends EditorPart implements IEditorPart {
	public ProtocolOutEditor() {
	}

	public static final String ID = "ProtocolOutTcpEditor"; //$NON-NLS-1$
	...
	private Text text_accptQueueSize;
	...
	public void createPartControl(Composite parent) {
		...
		if("TCP".equalsIgnoreCase(protocolType)){
			...
			text_ip.setBounds(130, 80, 150, 23);
			text_ip.setText(service.getIp() == null ? "" : service.getIp());
			formToolkit.adapt(text_ip, true, true);
			...
			text_port.setBounds(432, 77, 167, 23);
			...
			text_url.setBounds(130, 80, 364, 23);
			text_url.setText(service.getUrl() == null ? "" : service.getUrl());
		...
		text_accptQueueSize = new Text(composite_1, SWT.BORDER);
		text_accptQueueSize.setBounds(140, 91, 98, 23);
		text_accptQueueSize.setText(service.getAcceptQueueSize() == null ? "" : service.getAcceptQueueSize());
		formToolkit.adapt(text_accptQueueSize, true, true);
...
//		table = formToolkit.createTable(composite_2,  SWT.BORDER | SWT.V_SCROLL
//		        | SWT.H_SCROLL);
		...
		button_save.addMouseListener(new MouseListener() {
			...
			public void mouseUp(MouseEvent e) {
				...
				String accptQueueSize = text_accptQueueSize.getText();
				...
				ProtocolOutNode node = new ProtocolOutNode(service.getProtocolName());
	...
	public void init(IEditorSite site, IEditorInput input) throws PartInitException {
		...
		try {
			...
			File  localfile = new File(resource.getLocation().toOSString()); 
			freader = new FileInputStream(localfile);
			objectInputStream = new ObjectInputStream(freader);
			node = (ProtocolOutNode) objectInputStream.readObject();
