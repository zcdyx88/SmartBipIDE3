...
public class CustomServiceTaskContextImpl implements CustomServiceTaskContext {
	...
	public InputStream getSmallIconStream() {
		...
		if (path != null && null != extensionJarFile) {
	...
	private InputStream getDefaultCustomServiceTaskIconStream() {
		...
		try{
			inputStream = Thread.currentThread().getContextClassLoader().getResourceAsStream(customServiceTask.getSmallIconPath());
			if(null == inputStream){
				inputStream = Thread.currentThread().getContextClassLoader().getResourceAsStream(DEFAULT_ICON_PATH);
			}
		}catch(Exception e){
			inputStream = Thread.currentThread().getContextClassLoader().getResourceAsStream(DEFAULT_ICON_PATH);
		}
	...
	private InputStream getErrorCustomServiceTaskIconStream() {
		...
		try{
			inputStream = Thread.currentThread().getContextClassLoader().getResourceAsStream(customServiceTask.getSmallIconPath());
			if(null == inputStream){
				inputStream = Thread.currentThread().getContextClassLoader().getResourceAsStream(ERROR_ICON_PATH);
			}
		}catch(Exception e){
			inputStream = Thread.currentThread().getContextClassLoader().getResourceAsStream(ERROR_ICON_PATH);
		}
