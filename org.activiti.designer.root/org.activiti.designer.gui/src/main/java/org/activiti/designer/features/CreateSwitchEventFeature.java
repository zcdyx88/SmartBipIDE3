...
public class CreateSwitchEventFeature extends AbstractCreateFastBPMNFeature  {
	public static final String FEATURE_ID_KEY = "servicetask";
	private static final String SWITCH_EVENT =  "event.SwitchEvent";

	private String customServiceTaskId;

	public CreateSwitchEventFeature(IFeatureProvider fp) {
	    super(fp, "基础服务", "新增基础服务");
	  }

	public CreateSwitchEventFeature(IFeatureProvider fp, String name, String description, String customServiceTaskId) {
	...
	public Object[] create(ICreateContext context) {
		...
		newServiceTask.setName("基础服务");
//		newServiceTask.setExtensionId(customServiceTaskId);
		newServiceTask.setExtensionId(SWITCH_EVENT);
		...
		if (newServiceTask.isExtended()) {
			...
			if (targetTask != null) {
				...
				// What should happen if the class contain more than one
				// annotations?
				...
				for (final FieldInfo fieldInfo : fieldInfoObjects) {
					...
					CustomProperty customProperty = ExtensionUtil.getCustomProperty(newServiceTask,
					...
					if (localesAnnotation != null && localesAnnotation.value() != null
							&& localesAnnotation.value().length > 0) {
						String defaultLanguage = PreferencesUtil.getStringPreference(
								Preferences.ACTIVITI_DEFAULT_LANGUAGE, ActivitiPlugin.getDefault());
	...
	protected CustomServiceTask findCustomServiceTask(ServiceTask serviceTask) {
		...
		if (serviceTask.isExtended()) {
			...
			final List<CustomServiceTask> customServiceTasks = ExtensionUtil
					.getCustomServiceTasks(ActivitiUiUtil.getProjectFromDiagram(getDiagram()));
