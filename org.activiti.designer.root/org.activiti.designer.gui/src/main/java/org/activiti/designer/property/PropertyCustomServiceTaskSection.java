...
import org.activiti.designer.property.extension.field.CustomProperyBaseSvcChoiceField;
import org.activiti.designer.property.extension.field.CustomProperyBusiSvcChoiceField;
      ...
      if (targetTask != null) {
        ...
        for (final FieldInfo fieldInfo : fieldInfoObjects) {
          ...
          if (property.visible()) {
            ...
            case BASESVC_CHOICE:
            	currentProject = ActivitiUiUtil.getProjectFromDiagram(this.getDiagram());
                createdCustomPropertyField = new CustomProperyBaseSvcChoiceField(this, serviceTask, fieldInfo.getField(),currentProject);
                createdControl = createdCustomPropertyField.render(workParent, factory, listener);
                data = new FormData();
                data.top = new FormAttachment(previousAnchor, VSPACE);
                data.left = new FormAttachment(0, LABEL_COLUMN_WIDTH);
            ...
            case BUSISVC_CHOICE:
            	currentProject = ActivitiUiUtil.getProjectFromDiagram(this.getDiagram());
                createdCustomPropertyField = new CustomProperyBusiSvcChoiceField(this, serviceTask, fieldInfo.getField(),currentProject);
                createdControl = createdCustomPropertyField.render(workParent, factory, listener);
                data = new FormData();
                data.top = new FormAttachment(previousAnchor, VSPACE);
                data.left = new FormAttachment(0, LABEL_COLUMN_WIDTH);
